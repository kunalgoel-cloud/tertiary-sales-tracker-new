...
with st.sidebar:
    ...
    # THE REFRESH BUTTON: Clears Streamlit's cache to pull fresh data from GSheets
    ...
    st.divider()
    if st.checkbox("Show Data Management (Danger)"):
        st.warning("These actions are permanent on Google Sheets")
        if st.button("🗑️ Wipe All Sales History"):
            empty_df = pd.DataFrame(columns=["date", "channel", "item_name", "qty_sold", "revenue"])
            conn.update(spreadsheet=SHEET_URL, worksheet="sales", data=empty_df)
            st.cache_data.clear()
            st.success("Cloud History Wiped!"); st.rerun()

st.title("📈 Cloud-Synced Sales Dashboard")

# Helper function to read sheets safely
def load_sheet(name, columns):
    try:
        # ttl=0 ensures we don't use old cached data when the page loads
...
with tab1:
    ...
    if not history_df.empty:
        ...
        st.info("Database is empty. Upload sales data to begin.")

# --- TAB 2: SMART UPLOAD ---
with tab2:
    if master_chans.empty:
        st.warning("Add Sales Channels in the Configuration tab first.")
    else:
        channel = st.selectbox("Select Channel", master_chans['name'].tolist())
        ...
        if uploaded_file:
            ...
            st.markdown("### 🛠 Step 1: Columns")
            c1, c2, c3 = st.columns(3)
            ci = c1.selectbox("Product Name", cols, index=saved["item_col"])
            cq = c1.selectbox("Quantity", cols, index=saved["qty_col"])
            cr = c2.selectbox("Revenue", cols, index=saved["rev_col"])
            cd = c2.selectbox("Date", cols, index=saved["date_col"])
            cv = c3.selectbox("Variant (Optional)", cols, index=saved["var_col"])
            ...
            if ci != "None" and not master_skus.empty:
                ...
                for _, row in df_unique.iterrows():
                    ...
                    mapping_updates[key] = st.selectbox(f"Map '{key}'", m_list, 
                ...
                if st.button("🚀 Sync Data to Cloud"):
                    with st.spinner("Updating Google Sheets..."):
                        ...
                        # Save Item Map
                        new_mapping_rows = [[k, v] for k, v in mapping_updates.items()]
