...
with tab1:
    ...
    if not history_df.empty:
        ...
        min_db_date = history_df['date_dt'].min().date()
        max_db_date = history_df['date_dt'].max().date()
        today = datetime.now().date()

        st.subheader("Filters")
        
        # 1. QUICK TIME SELECTORS
        ...
        with f2:
            sel_item = st.multiselect("Filter Products (Syncs with Channels)", available_items)
        ...
        if not filtered.empty:
            ...
            if len(all_groups) > 0:
                ...
                plot_df = pd.merge(template, plot_df, on=['date', color_theme], how='left').fillna(0)
                ...
                if show_labels:
                    ...
                    fig.add_scatter(x=totals['date'], y=totals[target_col], 
                                    text=totals[target_col].apply(lambda x: f'{x:,.0f}' if x > 0 else ''),
...
# --- TAB 2: SMART UPLOAD (Previous Logic Retained) ---
with tab2:
    ...
    if uploaded_file:
        ...
        if res: saved = {k: (c_names.index(v) if v in c_names else 0) for k, v in zip(["item", "qty", "rev", "date"], res[1:])}
        ...
        if col_item != "None":
            ...
            for item in raw_items:
                ...
                if known: mapping_updates[item] = known[0]
                else: mapping_updates[item] = st.selectbox(f"Map '{item}' to Master SKU:", masters, key=f"up_{item}")

            if st.button("🚀 Process & Save Data"):
                c.execute("INSERT OR REPLACE INTO col_map VALUES (?, ?, ?, ?, ?)", (channel, col_item, col_qty, col_rev, col_date))
                for raw, master in mapping_updates.items(): c.execute("INSERT OR REPLACE INTO item_map VALUES (?, ?)", (raw, master))
                conn.commit()
                ...
                for d in upload_df['date'].unique(): c.execute("DELETE FROM sales WHERE date = ? AND channel = ?", (d, channel))
